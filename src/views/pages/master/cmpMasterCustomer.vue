        ...
        <div class="modal-body">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="col-md-6">
                <div class="form-group">
                  <label for="example-nf-email">Opsi Code</label>
                  <!-- <CmpInputText
                      id="inputA"
                      type="text"
                      placeholder="Input Opsi Code "
                  ...
                  <v-select
                    ...
                    label="cust_code"
                    name="cust_code"
                    :options="dataOpsiCode"
                    placeholder="Select Opsi Code"
                    :disabled="isFetching"
                    v-model="selectedOpsiCode"
                    @update:modelValue="onChangeOpsiCodeHandler"
                  ></v-select>
                </div>
              </div>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  <label for="example-nf-email">Area Code</label>
                  <v-select
                    id="select-area_code"
                    variant="outlined"
                    label="area_code"
...
export default {
  ...
  methods: {
    ...
    onChangeRegionNameHandler() {
      this.todo.region_name = this.selectedRegionName.region_name;
      this.getAllAreas();
    },
    ...
    async getAllDatas() {
      ...
      const urlGetRegionName =
        this.$root.apiHost + this.$root.prefixApi + "M_RegionAll";
      const { data } = await axios.get(urlGetRegionName);
      console.log(data, "SINI");
      this.dataRegionName = data.data;

      this.isFetching = false;
    },
    async getAllOpsiCode() {
      this.isFetching = true;

      const urlGetOpsiCode =
        this.$root.apiHost + this.$root.prefixApi + "M_OpsiCabangAll";
      const { data } = await axios.get(urlGetOpsiCode);
      console.log(data, "SINI");
      this.dataOpsiCode = data.data;

      this.isFetching = false;
    },

    onChangeOpsiCodeHandler() {
      this.todo.cust_code = this.selectedOpsiCode.cust_code;
    },
    onChangeAreaCodeHandler() {
      this.todo.area_code = this.selectedAreaCode.area_code;
    },

    // async getAllAreas() {
    //   this.isFetching = true;
    //   const params = { region_name: this.todo.region_name }
    //   const urlGetAreaCode =
    //     this.$root.apiHost + this.$root.prefixApi + "M_Areaget";
    //   const { data } = await axios.get(urlGetAreaCode);
    //   console.log(data, "SINI");
    //   this.dataAreaCode = data.data;

    //   this.isFetching = false;
    // },
    async getAllAreas() {
      this.isFetching = true;
      const params = { region_name: this.todo.region_name }; // Ambil params dari `this.todo.region_name`
      const urlGetAreaCode =
        this.$root.apiHost + this.$root.prefixApi + "M_Areaget";

      try {
        // Mengirimkan params sebagai bagian dari opsi di axios
        const { data } = await axios.get(urlGetAreaCode, { params });
        console.log(data, "SINI");
        this.dataAreaCode = data.data;
      } catch (error) {
        console.error("Error fetching area codes: ", error);
      } finally {
        this.isFetching = false; // Mengatur isFetching menjadi false meskipun terjadi error
      }
    },
    ...
    saveTodo() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const config = "";
          var url =
            mythis.$root.apiHost + mythis.$root.prefixApi + "M_Customer";
    ...
    saveTodoBulky() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const config = "";
          var url =
            mythis.$root.apiHost + mythis.$root.prefixApi + "M_CustomerBulky";
    ...
    async getDataExportExcel() {
      ...
      mythis.nama_excelnya = "MASTER_CUSTOMER_" + a + ".xlsx";
    ...
    async exportPdf() {
      ...
      try {
        ...
        doc.setFontSize(18);
        doc.text("Master Customer Report", 14, 22);
        doc.setFontSize(11);
        ...
        const fileName =
          "Master_Customer_Report_" + mythis.formatDate(new Date()) + ".pdf";
        doc.save(fileName);
    ...
    getTable() {
      ...
      this.grid.updateConfig({
        ...
        server: {
          url: this.$root.apiHost + this.$root.prefixApi + "M_Customer",
          then: (data) =>
    ...
    async getData(id) {
      ...
      await axios
        ...
        .then(async (res) => {
          ...
          mythis.todo.id = id;
          mythis.todo.dist_code = res.data.data.dist_code.toUpperCase();
          mythis.todo.region_name = res.data.data.region_name.toUpperCase();
          mythis.todo.area_code = res.data.data.area_code.toUpperCase();
          mythis.todo.kode_cabang = res.data.data.kode_cabang.toUpperCase();
          mythis.todo.cust_code = res.data.data.cust_code.toUpperCase();
          mythis.todo.cust_name = res.data.data.cust_name.toUpperCase();
          mythis.todo.chnl_code = res.data.data.chnl_code.toUpperCase();
          mythis.todo.item_code = res.data.data.item_code.toUpperCase();
          mythis.todo.kota = res.data.data.kota.toUpperCase();
          mythis.todo.provinsi = res.data.data.provinsi.toUpperCase();
