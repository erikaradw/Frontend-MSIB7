  ...
  methods: {
    ...
    async applyFilters() {
      ...
      const response = await axios.delete(
        this.$root.apiHost + this.$root.prefixApi + "deletefilterstockdetail",
        config
      );
    ...
    async deleteSelectedRows() {
      ...
      try {
        ...
        axios
          ...
          .delete(this.$root.apiHost + this.$root.prefixApi + `stock/hapus-banyak-data`, {
    ...
    deleteLastUploadedFile() {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          axios
            .delete(this.$root.apiHost + this.$root.prefixApi + "si/stockdetail")
            .then(() => {
              Swal.fire(
                "Deleted!",
      ...
      this.grid.updateConfig({
        ...
        style: {
          ...
          th: {
            ...
            "border-bottom": "1.5px solid #1a5b92", // Moderate border for header bottom
            "text-align": "center", // Center-align header text
            padding: "10px 16px", // Adjusted padding
            "font-size": "13px", // Slightly smaller font size
            "font-weight": "bold", // Bold font for header
            "white-space": "nowrap", // Prevent text wrapping
            "text-shadow": "0.5px 0.5px 1px rgba(0, 0, 0, 0.2)", // Subtle text shadow for depth
          },
          td: {
        ...
        server: {
          ...
          then: (data) => {
            // Gabungkan data baru dengan data yang sudah ada di penampung
            this.penampung = data.results; // Menggabungkan data lama dan baru

            // Return data untuk di-render
          ...
          handle: (res) => {
            if (res.status === 404) {
              return { data: [] }; // Jika tidak ada data
            }
            if (res.ok) {
              return res.json(); // Berhasil mengambil data
            }
            throw Error("oh no :("); // Error lain
          },
        },
      });
