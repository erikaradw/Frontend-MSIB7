        ...
        <div class="modal-body">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="col-md-6">
                <div class="form-group">
                  <label for="example-nf-email">Price Name</label>
                  <input
                    :class="
                      errorField.price_name
                        ? 'form-control input-lg input-error'
                        : 'form-control input-lg'
                    "
                    rows="4"
                    v-model="todo.price_name"
                    placeholder="Input Price Name"
                    @input="
                      (val) => (todo.price_name = todo.price_name.toUpperCase())
                    "
                  ></input>
                </div>
              </div>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  <label for="example-nf-email">Flag Active</label>
                  <input
                    id="inputA"
                    type="checkbox"
            ...
            <div class="col-xs-12">
              <button
                ...
                :disabled="
                  ...
                  todo.price_code == '' ||
                  todo.price_name == null ||
                  todo.price_name == '' ||
                  // todo.obj_type == null ||
                  // todo.obj_type == '' ||
                  todo.flag_active == null
...
export default {
  ...
  methods: {
    saveTodoBulk() {
      ...
      axios
        .post(
          mythis.$root.apiHost + mythis.$root.prefixApi + M_PriceinsertBulk,
          {
            csv: this.csvValid,
            user_id: this.userid,
          }
        ...
        if (result.isConfirmed) {
          ...
          try {
            const response = await axios.delete(
              mythis.$root.apiHost + mythis.$root.prefixApi + "M_Pricedelete",
              config
            );
            mythis.$root.stopLoading();
    ...
    saveTodo() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          //   },
          // };
          var url = mythis.$root.apiHost + mythis.$root.prefixApi + "M_Price";
          const config = "";
          axios
            .post(
              url,
              {
                ...
                price_name: mythis.todo.price_name,
    ...
    saveTodoBulky() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const config = "";
          var url =
            mythis.$root.apiHost + mythis.$root.prefixApi + "M_PriceBulky";
    ...
    async getDataExportExcel() {
      ...
      while (count > 0) {
        ...
        Object.keys(resData.results).forEach(function (key) {
          const countries_x = {
            ...
            priceName: resData.results[key].price_name,
            flagActive: resData.results[key].flag_active,
          };
      ...
      mythis.nama_excelnya = "MASTER_PRICE_" + a + ".xlsx";
    ...
    async exportPdf() {
      ...
      try {
        ...
        while (count > 0) {
          ...
          const reqData = await axios({
            method: 'get',
            url: mythis.$root.apiHost + 'si/M_Price?offset=' + offsetx + '&limit=' + limitx,
          });
        ...
        doc.setFontSize(18);
        doc.text('Master Price Report', 14, 22);
        doc.setFontSize(11);
        ...
        doc.autoTable({
          theme: 'striped',
          head: [['No', 'Price Code', 'Price Name', 'Active']],
          body: allData.map((item, index) => [index + 1, item.price_code, item.price_name, item.flag_active ? 'Yes' : 'No']),
        ...
        const fileName = 'Master_Price_Report_' + mythis.formatDate(new Date()) + '.pdf';
        doc.save(fileName);
    ...
    getTable() {
      ...
      this.grid.updateConfig({
        ...
        server: {
          url: this.$root.apiHost + this.$root.prefixApi + "M_Price",
          then: (data) =>
            data.results.map((card) => [
              card.id,
              data.nomorBaris++ + 1,
              html(`<span class="pull-left">${card.price_code}</span>`),
              html(`<span class="pull-left">${card.price_name}</span>`),
              // html(`<span class="pull-left">${card.obj_type}</span>`),
              html(`<span class="pull-left">${card.flag_active}</span>`),
            ]),
    ...
    deleteTodo(id) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          axios
            .delete(
              mythis.$root.apiHost + mythis.$root.prefixApi + `M_Price/${id}`,
              config
            )
            .then((res) => {
    ...
    async getData(id) {
      ...
      await axios
        ...
        .then(async (res) => {
          ...
          mythis.todo.id = res.data.data.id;
          mythis.todo.price_code = res.data.data.price_code;
          mythis.todo.price_name = res.data.data.price_name.toUpperCase();
