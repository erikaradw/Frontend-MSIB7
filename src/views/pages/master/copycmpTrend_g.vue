<template>
  ...
  export default {
    ...
    methods: {
          ...
          if (result.isConfirmed) {
            ...
            try {
              const response = await axios.delete(
                mythis.$root.apiHost + mythis.$root.prefixApi + "trend_gdelete",
                config
              );
      ...
      downloadCSV() {
        const url = `${this.$root.apiHost}${this.$root.prefixApi}trend_gBulky`; // Ganti dengan URL endpoint Anda
        axios({
          url: url,
          method: "GET",
      ...
      saveTodoBulky() {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            const config = "";
            var url =
              mythis.$root.apiHost + mythis.$root.prefixApi + "trend_gBulky";
            axios
              .post(
                url,
                {
      ...
      async exportPdf() {
        ...
        try {
          ...
          doc.setFontSize(18);
          doc.text("Master Trend Report", 14, 22);
          doc.setFontSize(11);
          ...
          const fileName =
            "Master_Trend_Report_" + mythis.formatDate(new Date()) + ".pdf";
          doc.save(fileName);
      ...
      getTable() {
        ...
        this.grid.updateConfig({
          ...
          server: {
            url: this.$root.apiHost + this.$root.prefixApi + "trend_g", // "trend",
            then: (data) =>
      ...
      async getData(id) {
        ...
        await axios
          ...
          .then(async (res) => {
            ...
            mythis.todo.parent = res.data.data.parent;
